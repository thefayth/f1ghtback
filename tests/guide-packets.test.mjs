...
test("California confirmation proceeds while cross-state uncertainty holds", () => {
  assert.equal(shouldHoldGuideStep("confirm-jurisdiction", "California case; no cross-state concern known"), false);
  assert.equal(shouldHoldGuideStep("confirm-jurisdiction", "Cross-state connection or unsure"), true);
  assert.equal(shouldHoldGuideStep("identify-papers", "Different document or unsure"), true);
});
